...
def _(cfg, data_path, mo):
    ...
    mo.md(f"""
    ## Splits
    | Split | Rows | Target Rate | % of Total |
    |---|---|---|---|
    | Train | {meta['train']['n_samples']:,} | {meta['train']['target_rate']:.1%} | {meta['train']['pct_total']:.1%} |
    | Calibration | {meta['calibration']['n_samples']:,} | {meta['calibration']['target_rate']:.1%} | {meta['calibration']['pct_total']:.1%} |
    | Test | {meta['test']['n_samples']:,} | {meta['test']['target_rate']:.1%} | {meta['test']['pct_total']:.1%} |
    """)
...
def _(
    ...
    with tracked_run(config_dict, data_path, cfg.experiment.name, run_name=cfg.experiment.run_name, mlflow_config=mlflow_config):
    ...
    |---|---|---|---|
    | **ROC-AUC** | {train_metrics["train_roc_auc"]:.4f} | {cal_metrics["cal_roc_auc"]:.4f} | {test_metrics["test_roc_auc"]:.4f} |
    ...
    | **Precision (macro)** | {train_metrics["train_precision_macro"]:.4f} | {cal_metrics["cal_precision_macro"]:.4f} | {test_metrics["test_precision_macro"]:.4f} |
    | **Recall (macro)** | {train_metrics["train_recall_macro"]:.4f} | {cal_metrics["cal_recall_macro"]:.4f} | {test_metrics["test_recall_macro"]:.4f} |
    """)
    return
