...
def _(cfg, json, mlflow, mo):
    ...
    runs = mlflow.search_runs(
        ...
        filter_string="tags.mlflow.runName = 'copa2026-conformal-evaluation'",
...
def _(
    ...
    fcod_features = {
        "lead_time":                {"label": "Lead Time (days)",  "clip": None},
        "arrival_date_week_number": {"label": "Arrival Week",       "clip": None},
        "adr":                      {"label": "Avg Daily Rate (€)", "clip": (0, float(X_test["adr"].quantile(0.99)))},
    }
...
def _(alpha, fcod_results_ci, np, plot_uncertainty_zones, plt):
    def _():
        ...
        plot_uncertainty_zones(
            fcod_results_ci["lead_time"],
            ax=ax,
            safe_threshold=0.7,
            uncertain_threshold=0.3,
            title=f"Decision Zones: Lead Time (α={alpha})",
        )

        lead_grid = fcod_results_ci["lead_time"]["grid"]
        lead_sc = fcod_results_ci["lead_time"]["SC"]
        safe_idx = np.where(lead_sc >= 0.7)[0]
        if len(safe_idx) > 0:
            safe_start = lead_grid[safe_idx[0]]
            print(f"Safe zone (SC >= 70%) begins at Lead Time ~= {safe_start:.0f}")
        else:
...
def _(alpha, fcod_ci_by_class, plot_uncertainty_zones, plt, unique_classes):
    def _():
        for cls in unique_classes:
            fig, ax = plt.subplots(figsize=(12, 5))
            plot_uncertainty_zones(
                fcod_ci_by_class[cls]["lead_time"], ax=ax,
                safe_threshold=0.7, uncertain_threshold=0.3,
                title=f"Decision Zones: Lead Time — Class {cls} (α={alpha})",
            )
            plt.tight_layout()
...
def _(
    ...
    def _():
        ...
        with tracked_run(config_dict, data_path, cfg.experiment.name, run_name="copa2026-fcod-analysis", mlflow_config=mlflow_config):
            ...
            # Uncertainty zones: lead_time
            fig_zones, ax = plt.subplots(figsize=(12, 5))
            plot_uncertainty_zones(
                fcod_results_ci["lead_time"], ax=ax,
                safe_threshold=0.7, uncertain_threshold=0.3,
                title=f"Decision Zones: Lead Time (α={alpha})",
            )
            ...
            for _cls in unique_classes:
                ...
                plot_uncertainty_zones(
                    _cls_fcod_ci["lead_time"], ax=_ax,
                    safe_threshold=0.7, uncertain_threshold=0.3,
                    title=f"Decision Zones: Lead Time — Class {_cls} (α={alpha})",
                )
