"""Generate a self-contained HTML report for the conformal-p2p experiment.

Fetches metrics and artifacts from MLflow, renders a Jinja2 template, saves the
report locally, and logs it as an MLflow artifact in a new ``p2p-experiment-report``
run.

Usage
-----
    uv run experiments/conformal-p2p/generate_report.py
"""
...
RUN_NAMES: dict[str, str] = {
    ...
    "conformal": "conformal-evaluation",
    "fcod":      "fcod-analysis",
    "shap":      "shap-margin-analysis",
}

# Primary FCOD artifact filenames (display order)
...
# SHAP artifact paths to display
SHAP_P2P_ARTIFACTS: dict[str, str] = {
    "p0_beeswarm_all":         "shap/beeswarm/pvalues/p0_all.png",
    "p1_beeswarm_all":         "shap/beeswarm/pvalues/p1_all.png",
    "margin_beeswarm_all":     "shap/beeswarm/derived/margin_all.png",
    "margin_beeswarm_class0":  "shap/beeswarm/derived/margin_class0.png",
    "margin_beeswarm_class1":  "shap/beeswarm/derived/margin_class1.png",
    ...
    "dep_numeric_margin":      "shap/dependence/numeric/margin.png",
    "dep_numeric_pvalues":     "shap/dependence/numeric/pvalues.png",
    "dep_numeric_cc":          "shap/dependence/numeric/confidence_credibility.png",
    "dep_cat_margin":          "shap/dependence/categorical/margin.png",
    "dep_cat_pvalues":         "shap/dependence/categorical/pvalues.png",
    "dep_cat_cc":              "shap/dependence/categorical/confidence_credibility.png",
...
def build_context(
    ...
    """
    # ── Stage 02: model ──────────────────────────────────────────────────────
    print(f"  model run: {RUN_NAMES['model']}")
    model_run = get_latest_run(client, experiment_id, RUN_NAMES["model"])
    ...
    model: dict[str, Any] = {
        ...
        "train_precision_macro":   mm.get("train_precision_macro"),
        "cal_precision_macro":     mm.get("cal_precision_macro"),
        "test_precision_macro":    mm.get("test_precision_macro"),
        "train_recall_macro":      mm.get("train_recall_macro"),
    ...
    # ── Stage 03: conformal ──────────────────────────────────────────────────
    print(f"  conformal run: {RUN_NAMES['conformal']}")
    conformal_run = get_latest_run(client, experiment_id, RUN_NAMES["conformal"])
    ...
    conformal: dict[str, Any] = {
        ...
        "test_si_fp_rate":              cm.get("test_si_fp_rate"),
        "test_si_fn_rate":              cm.get("test_si_fn_rate"),
        "cal_qhat":                     cal_qhat,
    }
    ...
    # ── Stage 04: FCOD ───────────────────────────────────────────────────────
    print(f"  FCOD run: {RUN_NAMES['fcod']}")
    fcod_run = get_latest_run(client, experiment_id, RUN_NAMES["fcod"])
    fcod_artifacts: dict[str, str | None] = {}

    if fcod_run:
        run_id = fcod_run.info.run_id
        for artifact_name in FCOD_ARTIFACT_NAMES:
            artifact_key = artifact_name.replace(".png", "")
            fcod_artifacts[artifact_key] = download_artifact_b64(run_id, artifact_name)
        ...
        for path in all_paths:
            ...
            if filename.endswith(".png"):
                ...
                if artifact_key not in fcod_artifacts:
                    fcod_artifacts[artifact_key] = download_artifact_b64(run_id, path)

        # Collect per-class artifacts from by_class/ subdirs
        fcod_artifacts_by_class: dict[str, dict[str, str | None]] = {}
        class_dirs = sorted({
            "/".join(p.split("/")[:2])
            for p in all_paths
        ...
        # Collect merged/ top-level artifacts (excluding merged/by_class/)
        fcod_artifacts_merged: dict[str, str | None] = {}
        ...
        for path in merged_top_paths:
            filename = Path(path).name
            art_key = filename.replace(".png", "")
            if art_key not in fcod_artifacts_merged:
                fcod_artifacts_merged[art_key] = download_artifact_b64(run_id, path)

        # Collect merged per-class artifacts
        fcod_artifacts_merged_by_class: dict[str, dict[str, str | None]] = {}
        ...
        for class_dir in merged_class_dirs:
            ...
            if class_arts_m:
                fcod_artifacts_merged_by_class[class_label] = class_arts_m

        # Collect per-feature stacked + density artifacts
        fcod_artifacts_stacked_density: dict[str, str | None] = {}
        sd_paths = sorted(
            p for p in all_paths
        ...
        for path in sd_paths:
            feat_name = Path(path).stem
            fcod_artifacts_stacked_density[feat_name] = download_artifact_b64(run_id, path)
    else:
        fcod_artifacts_by_class = {}
        fcod_artifacts_by_class_stacked = {}
        fcod_artifacts_merged = {}
        fcod_artifacts_merged_by_class = {}
        fcod_artifacts_stacked_density = {}

    # ── Stage 06: SHAP ───────────────────────────────────────────────────────
    print(f"  SHAP run: {RUN_NAMES['shap']}")
    shap_run = get_latest_run(client, experiment_id, RUN_NAMES["shap"])
    shap_artifacts: dict[str, str | None] = {}

    if shap_run:
        for art_key, art_path in SHAP_P2P_ARTIFACTS.items():
    ...
    return {
        ...
        "fcod_artifacts_merged": fcod_artifacts_merged,
        "fcod_artifacts_merged_by_class": fcod_artifacts_merged_by_class,
        "fcod_artifacts_stacked_density": fcod_artifacts_stacked_density,
...
def main() -> None:
    ...
    with mlflow.start_run(experiment_id=experiment_id, run_name="p2p-experiment-report"):
