...
def _(cfg, json, mlflow, mo):
    ...
    runs = mlflow.search_runs(
        ...
        filter_string="tags.mlflow.runName = 'globafa2026-conformal-evaluation'",
...
def _(
    ...
    fcod_features = {
        ...
        "dti_n":     {"label": "Debt-to-Income (%)",  "clip": (0, float(X_test["dti_n"].quantile(0.99)))},
        "loan_amnt": {"label": "Loan Amount ($)",     "clip": (0, float(X_test["loan_amnt"].quantile(0.99)))},
        "revenue":   {"label": "Annual Income ($)",   "clip": (0, float(X_test["revenue"].quantile(0.99)))},
    }
...
def _(alpha, fcod_results_ci, np, plot_uncertainty_zones, plt):
    def _():
        ...
        if len(safe_idx) > 0:
            ...
            print(f"Safe zone (SC >= 70%) begins at FICO ~= {safe_start:.0f}")
        else:
...
def _(X_test, alpha, np, plt, prediction_sets, y_test):
    ...
    cat_features = {
        "emp_length":      {"label": "Employment Length", "top_n": None, "sort_by": "alpha",   "ascending": True,  "figsize": (12, 5), "count_rotation": 45,
                            "category_order": EMP_LENGTH_ORDER, "sort_description": "ordinal order (NI last)"},
        "purpose":         {"label": "Loan Purpose",      "top_n": None, "sort_by": "sc_rate", "ascending": False, "figsize": (14, 5), "count_rotation": 45,
                            "sort_description": "ordered by SC rate ↓"},
        "home_ownership_n":{"label": "Home Ownership",    "top_n": None, "sort_by": "sc_rate", "ascending": False, "figsize": (8,  5), "count_rotation": 0,
                            "sort_description": "ordered by SC rate ↓"},
        "addr_state":      {"label": "State",             "top_n": 20,   "sort_by": "sc_rate", "ascending": False, "figsize": (14, 5), "count_rotation": 90,
                            "sort_description": "ordered by SC rate ↓ (Other last)"},
    }
    ...
    def styled_outcome_plot(feature_name, config, X, y, ps, suffix="",
        ...
        if show_title:
            sort_desc = config.get("sort_description")
            title = f"{config['label']}{suffix} (α={alpha})"
            if sort_desc:
                title = f"{title} — {sort_desc}"
            ax.set_title(title)
        fig.tight_layout()
        return fig
    ...
    GRID_FEATURES = ["emp_length", "purpose", "home_ownership_n", "addr_state"]
...
def _(
    ...
    def _():
        ...
        with tracked_run(config_dict, data_path, cfg.experiment.name, run_name="globafa2026-fcod-analysis", mlflow_config=mlflow_config):
            ...
            # Uncertainty zones: fico_n
            fig_zones, ax = plt.subplots(figsize=(12, 5))
            plot_uncertainty_zones(
                fcod_results_ci["fico_n"], ax=ax,
                safe_threshold=0.7, uncertain_threshold=0.3,
                title=f"Decision Zones: FICO Score (α={alpha})",
            )
            mlflow.log_figure(fig_zones, "uncertainty_zones_fico_n.png", save_kwargs={"bbox_inches": "tight"})
            plt.close(fig_zones)
            ...
            for _cls in unique_classes:
                ...
                mlflow.log_figure(_fig_zones, f"{_prefix}/uncertainty_zones_fico_n.png", save_kwargs={"bbox_inches": "tight"})
