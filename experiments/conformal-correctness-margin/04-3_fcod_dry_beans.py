...
def _(cfg, json, mlflow, mo):
    ...
    runs = mlflow.search_runs(
        ...
        filter_string="tags.mlflow.runName = 'dry-beans-conformal-evaluation'",
...
def _(
    ...
    # Size/length features with right skew → (q01, q99)
    _both_clip = {"area", "perimeter", "majoraxislength", "minoraxislength",
                  "convexarea", "equivdiameter"}

    # Normalized ratios and shape factors → no clip
    _no_clip = {"eccentricity", "aspectratio", "extent", "solidity",
...
def _(alpha, fcod_results_ci, np, plot_uncertainty_zones, plt):
    def _():
        ...
        plot_uncertainty_zones(
            fcod_results_ci["eccentricity"],
            ax=ax,
            safe_threshold=0.7,
            uncertain_threshold=0.3,
            title=f"Decision Zones: Eccentricity (α={alpha})",
        )

        ecc_grid = fcod_results_ci["eccentricity"]["grid"]
        ecc_sc = fcod_results_ci["eccentricity"]["SC"]
        safe_idx = np.where(ecc_sc >= 0.7)[0]
        if len(safe_idx) > 0:
            safe_start = ecc_grid[safe_idx[0]]
            print(f"Safe zone (SC >= 70%) begins at Eccentricity ~= {safe_start:.3f}")
        else:
...
def _(
    ...
    def _():
        ...
        with tracked_run(config_dict, data_path, cfg.experiment.name, run_name="dry-beans-fcod-analysis"):
            ...
            # Uncertainty zones: eccentricity
            fig_zones, ax = plt.subplots(figsize=(12, 5))
            plot_uncertainty_zones(
                fcod_results_ci["eccentricity"], ax=ax,
                safe_threshold=0.7, uncertain_threshold=0.3,
                title=f"Decision Zones: Eccentricity (α={alpha})",
            )
            mlflow.log_figure(fig_zones, "uncertainty_zones_eccentricity.png")
            plt.close(fig_zones)
