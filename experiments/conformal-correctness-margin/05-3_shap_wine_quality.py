...
def _():
    ...
    mo.md("""
# SHAP Margin Analysis — Wine Quality (conformal-correctness-margin)

SHAP explainability on the conformal classifier for the Wine Quality dataset.
All 12 features (11 numeric + 1 categorical) included via integer encoding.
Multiclass task — margin derived via `derive_margin_shap_multiclass`.
Results logged to MLflow as run `wine-shap-analysis`.
    """)
...
def _(X_explain, full_pvalue_func, mo, n_classes, n_explain, np, shap_kernel, y_explain):
    from conformalpy.explainability import derive_margin_shap_multiclass, derive_confidence_shap

    shap_per_class = [shap_kernel[:, :, k] for k in range(n_classes)]
    p_values_explain = full_pvalue_func(X_explain)

    # Multiclass: margin = p_true - p_best_false (per-sample best-false selection)
    shap_margin = derive_margin_shap_multiclass(shap_per_class, p_values_explain, y_explain)
    ...
    return (
        derive_confidence_shap, derive_margin_shap_multiclass, max_class, p_values_explain,
        shap_confidence, shap_credibility, shap_margin, shap_per_class,
    )
...
def _(
    ...
    # Top-4 numeric by importance (wine has 11 numeric)
    ...
    # Top-N categorical by importance (wine has only 1 categorical — wine_type)
    ...
    # Build tab10 class colors for multiclass
    _unique_classes = np.unique(y_explain)
    _tab10 = plt.cm.tab10
    _class_colors_mc = {
        int(_cls): _tab10(_ci / max(len(_unique_classes) - 1, 1))
        for _ci, _cls in enumerate(_unique_classes)
    }

    # Numeric 2×2 dependence (top-4) — multiclass, no proportion strip
    ...
    for _i, _col in enumerate(_top_num_cols):
        ...
        plot_shap_dependence_numeric(
            ...
            y_explain, _col, shap_label="margin",
            class_colors=_class_colors_mc,
            show_class_proportion=False,
            proportion_ax=_prop_ax,
        )
    fig_dep_margin_num.suptitle("SHAP(Margin) Dependence — Top-4 Numeric Features", fontsize=12)
    fig_dep_margin_num.tight_layout()

    # Categorical grid — wine has 1 categorical (wine_type), no proportion strip
    ...
    for _jj, (_col, _feat_g) in enumerate(zip(_top_cat_cols, _top_cat_glob_idx)):
        ...
        plot_shap_dependence_categorical(
            ...
            y_explain, _unique_labels, _col, shap_label="margin",
            class_colors=_class_colors_mc,
            show_class_proportion=False,
            proportion_ax=_prop_ax,
        )
...
def _(
    ...
    fig_heatmap, _ax_hm = plt.subplots(figsize=(10, 6))
    sns.heatmap(
...
def _(
    ...
    with tracked_run(_config_dict, data_path, cfg.experiment.name, run_name="wine-shap-analysis", mlflow_config=mlflow_config):
        mlflow.log_params({
...
Run `wine-shap-analysis` logged to experiment `conformal-correctness-margin`.
...
| `shap/dependence/categorical/` | wine_type dependence plot |
