...
def _(cfg, json, mlflow, mo):
    ...
    runs = mlflow.search_runs(
        ...
        filter_string="tags.mlflow.runName = 'credit-card-conformal-evaluation'",
...
def _(
    ...
    # Category A: unbounded or no clipping needed
    _no_clip = {"age"}

    # Category B: right-skewed, non-negative → (0, q99)
    _right_clip = {"limit_bal"} | {f"pay_amt{i}" for i in range(1, 7)}

    # Category C: can go negative, right-skewed → (q01, q99)
    _both_clip = {f"bill_amt{i}" for i in range(1, 7)}
    ...
    for col in cfg.data.features.numerical:
        ...
        if col in _no_clip:
            clip = None
        elif col in _right_clip:
            clip = (0.0, float(X_test[col].quantile(0.99)))
        elif col in _both_clip:
...
def _(alpha, fcod_results_ci, np, plot_uncertainty_zones, plt):
    def _():
        ...
        plot_uncertainty_zones(
            fcod_results_ci["limit_bal"],
            ax=ax,
            safe_threshold=0.7,
            uncertain_threshold=0.3,
            title=f"Decision Zones: Credit Limit (α={alpha})",
        )

        limit_grid = fcod_results_ci["limit_bal"]["grid"]
        limit_sc = fcod_results_ci["limit_bal"]["SC"]
        safe_idx = np.where(limit_sc >= 0.7)[0]
        if len(safe_idx) > 0:
            safe_start = limit_grid[safe_idx[0]]
            print(f"Safe zone (SC >= 70%) begins at Credit Limit ~= {safe_start:,.0f} NT$")
        else:
...
def _(X_test, alpha, math, np, plt, prediction_sets, y_test):
    ...
    cat_features = {
        ...
        "education": {"label": "Education Level",  "top_n": None},
        "marriage":  {"label": "Marital Status",   "top_n": None},
        "pay_0":     {"label": "Pay Status (Sep)", "top_n": None},
        "pay_2":     {"label": "Pay Status (Aug)", "top_n": None},
        "pay_3":     {"label": "Pay Status (Jul)", "top_n": None},
        "pay_4":     {"label": "Pay Status (Jun)", "top_n": None},
        "pay_5":     {"label": "Pay Status (May)", "top_n": None},
        "pay_6":     {"label": "Pay Status (Apr)", "top_n": None},
    }
    N_CAT_COLS = 3
...
def _(alpha, fcod_ci_by_class, plot_uncertainty_zones, plt, unique_classes):
    def _():
        for cls in unique_classes:
            fig, ax = plt.subplots(figsize=(12, 5))
            plot_uncertainty_zones(
                fcod_ci_by_class[cls]["limit_bal"], ax=ax,
                safe_threshold=0.7, uncertain_threshold=0.3,
                title=f"Decision Zones: Credit Limit — Class {cls} (α={alpha})",
            )
            plt.tight_layout()
...
def _(
    ...
    def _():
        ...
        with tracked_run(config_dict, data_path, cfg.experiment.name, run_name="credit-card-fcod-analysis", mlflow_config=mlflow_config):
            ...
            # Uncertainty zones: credit limit
            fig_zones, ax = plt.subplots(figsize=(12, 5))
            plot_uncertainty_zones(
                fcod_results_ci["limit_bal"], ax=ax,
                safe_threshold=0.7, uncertain_threshold=0.3,
                title=f"Decision Zones: Credit Limit (α={alpha})",
            )
            mlflow.log_figure(fig_zones, "uncertainty_zones_limit_bal.png", save_kwargs={"bbox_inches": "tight"})
            plt.close(fig_zones)
            ...
            for _cls in unique_classes:
                ...
                plot_uncertainty_zones(
                    _cls_fcod_ci["limit_bal"], ax=_ax,
                    safe_threshold=0.7, uncertain_threshold=0.3,
                    title=f"Decision Zones: Credit Limit — Class {_cls} (α={alpha})",
                )
                mlflow.log_figure(_fig_zones, f"{_prefix}/uncertainty_zones_limit_bal.png", save_kwargs={"bbox_inches": "tight"})
